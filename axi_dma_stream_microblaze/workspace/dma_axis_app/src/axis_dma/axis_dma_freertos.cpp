...
#define NUM_TX_PACKETS 			2
#define RX_PKT_SIZE 			768
...
__attribute__((section(".axi_bram_ctrl_0_Mem0"))) uint32_t txBD[16 *2] __attribute__ ((aligned (64)));
...
static void RxCallBack(XAxiDma_BdRing * RxRingPtr)
{
	...
	for (Index = 0; Index < BdCount; Index++)
	{
		...
		xQueueSendFromISR(gRxPktQueue, &rxPkt, &xHigherPriorityTaskWoken);
	...
	while (BdCount > 0)
	{
		...
		XAxiDma_BdSetBufAddr(BdPtr, RxBufferPtr);
		...
		Xil_DCacheFlushRange((INTPTR)BdPtr, sizeof( *BdPtr));
#endif
		status = XAxiDma_BdRingToHw(RxRingPtr, 1, BdPtr);
		if (status != XST_SUCCESS)
		{
			XAxiDma_BdRingUnAlloc(RxRingPtr, 1, BdPtr);
			gRxPktRBuffer.push(RxBufferPtr);
			break;
		}
	}
...
bool setupDMA()
{
	...
	gRxPktQueue = xQueueCreate(NUM_RX_PACKETS, sizeof(rxPkt_t));
