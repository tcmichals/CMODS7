...
typedef struct 
{
    ...
    uint32_t m_values[0]; //read values
}operationPayload_t;
...
readWriteMemoryProtocolServer::readWriteMemoryProtocolServer(basicProtocol *protocol): m_protocol(protocol)
{
   ...
   auto callback = basicProtocol::postPayloadCallback_t::make<&readWriteMemoryProtocolServer::postFromBasicProtocol>(*this);
    m_protocol->registerHandler(READ_WRITE_PROTOCOL_PORT, callback);
}
inline
...
inline
bool readWriteMemoryProtocolServer::bind(basicProtocol *protocol)
{
	m_protocol = protocol;

	auto callback = basicProtocol::postPayloadCallback_t::make<&readWriteMemoryProtocolServer::postFromBasicProtocol>(*this);
	m_protocol->registerHandler(READ_WRITE_PROTOCOL_PORT, callback);
	if ( XST_SUCCESS == XGpio_Initialize(&gXGPIO, 0))
	{
		XGpio_SetDataDirection(&gXGPIO,1, 0x0);
		return true;
	}
	else
		return false;
...
bool readWriteMemoryProtocolServer::postFromBasicProtocol( uint8_t from_port, uint8_t *pPayload, size_t sizeOfPayload)
{
    ...
    switch( static_cast<typeOfReadOrWrite>(pOperation->m_operation))
    {
        case typeOfReadOrWrite::ContiguousRead:
        {
           ...
           //TCM m_protocol->send_packet(from_port, READ_WRITE_PROTOCOL_PORT, reinterpret_cast<uint8_t *>(&m_payload), sizeof(m_payload) );
        ...
        case typeOfReadOrWrite::ContiguousWrite:
        {
            uint32_t *pMemory = reinterpret_cast<uint32_t *>(pOperation->m_address);
           	XGpio_DiscreteWrite(&gXGPIO, 1, *pValues++);
         	XGpio_DiscreteWrite(&gXGPIO, 2, 0x0);
